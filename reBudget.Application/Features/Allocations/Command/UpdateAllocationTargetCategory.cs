...
{
    public class UpdateAllocationTargetCategory
    {
        ...
        public class Handler : IRequestHandler<Command, Result>
        {
            ...
            public async Task<Result> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                if (!await _accessControlService.HasBudgetCategoryAccessAsync(request.TargetBudgetCategoryId))
                {
                ...
                var oldBudgetCategory = await _writeDbContext.BudgetCategories
                                                             .FirstAsync(x => x.BudgetCategoryId == allocation.TargetBudgetCategoryId, cancellationToken: cancellationToken);

                var newBudgetCategory = await _writeDbContext.BudgetCategories
                                                          .FirstAsync(x => x.BudgetCategoryId == request.TargetBudgetCategoryId, cancellationToken: cancellationToken);
                ...
                var oldCategoryId = allocation.TargetBudgetCategoryId;
                allocation.SetTargetBudgetCategory(newBudgetCategory);
                ...
                return new Result()
                       {
                           Id = allocation.TargetBudgetCategoryId
                       };
            }
        }
    }
